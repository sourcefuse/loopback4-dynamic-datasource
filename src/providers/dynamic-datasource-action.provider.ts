...
import {asMiddleware, Middleware, RequestContext, RestApplication, RestMiddlewareGroups} from '@loopback/rest';
...
export class DynamicDatasourceActionProvider implements Provider<SetupDatasourceFn> {
  constructor(
  ...
  async action(requestCtx: RequestContext) {
    ...
    const promises = Object.entries(datasourceProvider).map(async value => {
      ...
      if (!this.application.isBound(dbBindKey)) {
        ...
        this.application.bind(dbBindKey).to(datasource).tag(RepositoryTags.DATASOURCE);
      }
      requestCtx
  ...
  private async bindDatabaseIdentifier(requestCtx: RequestContext) {
    ...
    if (datasourceIdentifier) {
      await requestCtx.bind(DynamicDatasourceBindings.CURRENT_INDETIFIER).to(datasourceIdentifier);
    }
